...
->end()
    ...
    private function addSettingsSection(): NodeDefinition
    {
        return (new TreeBuilder('settings'))->getRootNode()
            ->addDefaultsIfNotSet()
            ->children()
                ->scalarNode('app_redis')
->cannotBeEmpty()
->end()
                ->scalarNode('app_cache_proxy_enabled')
->defaultTrue()
->end()
                ->scalarNode('user_entity_class')
->defaultValue('App\\Entity\\User')
->end()
                ->scalarNode('app_entity_namespace')
->defaultValue('App\\Entity')
->end()
                ->scalarNode('app_value_object_namespace')
->defaultValue('App\\Model\\ValueObject')
->end()
                ->scalarNode('app_enum_namespace')
->defaultValue('App\\Model\\Enum')
->end()
                ->integerNode('mongo_query_max_time_ms')
->defaultValue(ApiQueryMongo::DEFAULT_QUERY_MAX_TIME_MS)->end()
                ->booleanNode('send_context_id_with_response')
->defaultFalse()
->end()
...
->end()
    ...
    private function addHealthCheckSection(): NodeDefinition
    {
        return (new TreeBuilder('health_check'))->getRootNode()
            ...
            ->canBeDisabled()
            ->children()
                ->scalarNode('mysql_table_name')
->defaultValue('_doctrine_migration_versions')
->end()
...
->end()
    ...
    private function addJobsSection(): NodeDefinition
    {
        return (new TreeBuilder('jobs'))->getRootNode()
            ->addDefaultsIfNotSet()
            ->children()
                ->scalarNode('max_exec_time')
->defaultValue(50)
->end()
                ->scalarNode('max_memory')
->defaultValue(100_000_000)
->end()
                ->scalarNode('no_job_idle_time')
->defaultValue(10)
->end()
            ->end()
        ;
